...
function transformCommonjs(parse, code, id, isEntry, ignoreGlobal, ignoreRequire, customNamedExports, sourceMap, allowDynamicRequire, astCache, hasEsDefaultExport) {
  ...
  function transformAndCheckExports(code, id) {
    {
      ...
      const transformed = transformCommonjs(this.parse, code, id, this.getModuleInfo(id).isEntry, ignoreGlobal, ignoreRequire, customNamedExports[id], sourceMap, allowDynamicRequire, ast, hasDefaultExport);
    ...
    load(id) {
      ...
      if (id.endsWith(PROXY_SUFFIX)) {
        ...
        return getIsCjsPromise(actualId).then(isCjs => {
          if (esModulesWithDefaultExport.has(actualId)) return `export {default} from ${JSON.stringify(actualId)};`;else if (isCjs) return `import { __moduleExports } from ${JSON.stringify(actualId)}; export default __moduleExports;`;else if (esModulesWithoutDefaultExport.has(actualId)) return `import * as ${name} from ${JSON.stringify(actualId)}; export default ${name};`;else return `import * as ${name} from ${JSON.stringify(actualId)}; import {getCjsExportFromNamespace} from "${HELPERS_ID}"; export default getCjsExportFromNamespace(${name})`;
        });
      }
    },
